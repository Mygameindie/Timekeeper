// ===========================================================
// 🛝 PLAYGROUND MODE
// Pet stands still and plays with balls.
// Drag balls at it — it'll jump when hit!
...
(() => {
  ...
  const baseSet = {
    ...
    fly0: loadImg('base.png'),
    fly1: loadImg('base.png'),
    fall: loadImg('base.png'),
  };
  ...
  function onMove(e) {
    ...
    if (draggedBall) {
      ...
      balls.forEach(ball => {
        ...
        if (wasHover !== ball.hovering) canvas.style.cursor = ball.hovering ? 'grab' : 'default';
      });
  ...
  addBallBtn.addEventListener('click', () => {
    balls.push(makeBall(canvas.width * (0.2 + Math.random() * 0.6), canvas.height * 0.15));
  });
  ...
  function updatePets() {
    for (const p of pets) {
      ...
      for (const ball of balls) {
        const dx = ball.x - p.x;
        const dy = ball.y - (p.y - PET_H * 0.15);
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < BALL_R + 90 && p.onGround && p.jumpCooldown === 0) {
          p.vy = -20;
          p.onGround = false;
          p.jumpCooldown = 50;
          ...
          ball.vy = -12;
          if (window.PetStats && typeof window.PetStats.playground === 'function') window.PetStats.playground(0);
        }
      }
      p.frameTimer++;
      if (p.frameTimer > 8) {
  ...
  function getPetState(p) {
    if (!p.onGround) return p.vy > 5 ? 'fall' : (p.frame === 0 ? 'fly0' : 'fly1');
    return 'stand';
  }

  function drawPets() {
    for (const p of pets) {
      const state = getPetState(p);
      const image = baseSet[state];
      safeDraw(image, p.x - PET_W / 2, p.y - PET_H / 2, PET_W, PET_H);
      if (typeof window.drawOutfitOverlay === 'function') {
        window.drawOutfitOverlay(ctx, state, p.x - PET_W / 2, p.y - PET_H / 2, PET_W, PET_H, 0);
      }
    }
  }
