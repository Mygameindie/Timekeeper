...
(() => {
  ...
  // Inject minimal CSS so animation always works
  ...
  function getTargetPet(p) {
    for (let i = pets.length - 1; i >= 0; i--) {
      if (isOpaqueHit(p, pets[i], i)) return i;
    }
  ...
  function disgustPet(idx, recoilMs, hurtMs) {
    ...
    if (typeof window.setActivePet === "function") {
      window.setActivePet(idx);
    }

    if (window.PetStats) {
      window.PetStats.troll(idx);
    }
  }
  ...
  function draw() {
    ...
    for (let i = 0; i < pets.length; i++) {
      ...
      const set = baseSets[i] || baseSets[0];
      const img = wantHurt ? set.hurt : set.normal;

      if (img && !img._failed && img.complete && img.naturalWidth > 0) {
        ctx.save();
        ctx.filter = pet.drawFilter || "none";
