...
// HD/Retina canvas enabled. Single pet only.
// ===========================================================
(function () {
  ...
  function getCanvasW() { return window.innerWidth; }
  function getCanvasH() { return window.innerHeight; }
  function getFenceY() { return getCanvasH() * FENCE_Y_RATIO; }
    ...
    try {
      ...
      if (Array.isArray(data.patches)) {
        data.patches.forEach((p, i) => {
          if (Array.isArray(p) && patches[i]) patches[i] = p.map(pl => ({ ...pl }));
        });
      }
    } catch (_) {}
  }
  ...
  function buildToolbar() {
    ...
    [
      ...
      btn.addEventListener('click', () => { selectedTool = t.key; updateHighlights(); });
    ...
    crops.forEach(crop => {
      ...
      btn.addEventListener('click', () => { selectedCrop = crop.key; selectedTool = 'plant'; updateHighlights(); });
      toolbar.appendChild(btn);
    });
  ...
  let dragPos = null;
  function onDown(e) { dragPos = canvasPos(e); e.preventDefault(); }
  function onMove(e) { if (!dragPos) return; dragPos = canvasPos(e); if (e.touches) e.preventDefault(); }
  function onUp() { if (!dragPos) return; const p = dragPos; dragPos = null; handleAction(p); }
  function pointInPatch(p, patch) { return p.x >= patch.x && p.x <= patch.x + patch.w && p.y >= patch.y && p.y <= patch.y + patch.h; }
  ...
  function handleAction(p) {
    ...
    for (let i = 0; i < dirtPatches.length; i++) {
      ...
      if (selectedTool === 'plant' && selectedCrop) {
        patches[i].push({ crop: selectedCrop, plantedAt: Date.now(), stage: 'seedling', offX: (Math.random() - 0.5) * (patch.w * 0.55) });
        saveGardenState();
      } else if (selectedTool === 'water') {
        patches[i].forEach(plant => { plant.stage = 'ready'; plant.plantedAt = Date.now() - SEC_READY * 1000; });
        saveGardenState();
        ...
        ready.forEach((plant, idx) => {
          ...
          setTimeout(() => { flyToBag(sx, sy, plant.crop, null); }, idx * 140);
        });
  ...
  function drawLockedPet(fenceY) {
    const petH = Math.min(getCanvasH() * 0.28, 210);
    const petW = petH * (400 / 450);
    const petY = fenceY - petH - 10;
    const x = getCanvasW() * 0.5 - petW / 2;
    if (petImg && petImg.complete && petImg.naturalWidth > 0 && !petImg._failed) {
      ctx.save();
      ctx.imageSmoothingEnabled = true;
      ctx.imageSmoothingQuality = 'high';
      ctx.drawImage(petImg, x, petY, petW, petH);
      if (typeof window.drawOutfitOverlay === 'function') window.drawOutfitOverlay(ctx, 'stand', x, petY, petW, petH, 0);
      ctx.restore();
    }
  }
    ...
    dirtPatches.forEach((patch, pi) => {
      patches[pi].forEach(plant => {
        ...
        if (plant.stage === 'ready' && cropImgs[plant.crop]) {
          ...
          if (fi && fi.complete && fi.naturalWidth > 0) ctx.drawImage(fi, px - 14, py - size - 20, 28, 28);
        }
  ...
  function onResize() { resizeCanvasHD(); }
  ...
  buildBag();
  loadCrops().catch(err => { console.error(err); buildToolbar(); });
  loop();
